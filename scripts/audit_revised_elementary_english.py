#!/usr/bin/env python3
"""Strict release audit for the 371 revised elementary-English detail pages.
...
SITEMAP_PRESERVED_SUBJECT_SLUGS = {
    "고등영어학원",
    "고등수학학원",
    "중등영어학원",
    "중등수학학원",
...
# in the source H1. The safe focus contract remains independent from the
# generator so source drift cannot silently restore those fragments.
SOURCE_H1_CONTAMINATION_CONTRACT: dict[str, tuple[str, str]] = {
    "구의동": (
        "내신과 모의고사를 함께 준비하는 학습",
        "초등 영어 어휘와 문장 이해를 함께 준비하는 학습",
    ),
    "북가좌동": (
        "모의고사 시간 관리까지 함께 살펴보기",
        "초등 영어 읽기 속도와 이해를 함께 살피는 방법",
    ),
    "진관동": (
        "내신과 모의고사를 함께 준비하는 방법",
        "초등 영어 기초와 학교 영어 학습을 함께 준비하는 방법",
    ),
    "탄벌동": (
        "수학을 처음 배우는 아이의 기초 학습 안내",
        "초등 영어 기초 읽기와 어휘를 확인하는 방법",
    ),
    "권선동": (
        "예비고1 수학 개념까지 점검하는 학습관리",
        "초등 영어 문장 읽기와 기초 개념을 점검하는 방법",
    ),
    "이충동": (
        "예비고1 영어 준비",
        "초등 영어 기초를 다음 학년 학습으로 연결하는 방법",
    ),
    "쌍용동": (
        "고등 어휘 학습 관리",
        "초등 영어 어휘를 문장 읽기로 연결하는 학습 관리",
    ),
    "달동": (
        "예비고1 겨울방학 영어 준비",
        "초등 영어 겨울방학 기초 학습 순서를 정하는 방법",
    ),
    "원주혁신도시": (
        "수능 영어의 기초를 설계하는 방법",
        "초등 영어 읽기·어휘 기초를 설계하는 방법",
    ),
}
...
SOURCE_MIDDLE_TRANSITION_CONTRACT: dict[str, tuple[str, str]] = {
    "삼각산동": (
        "중1 첫 시험을 준비하는 방법",
        "초6 영어 기초에서 중학교 첫 시험 준비로 넘어가는 방법",
    ),
    "보라동": (
        "중등 내신을 준비하는 학습기록",
        "초6 영어 학습 기록으로 중학교 준비를 시작하는 방법",
    ),
    "영덕동": (
        "중학교 입학 전 기초를 준비하는 방법",
        "중학교 입학 전 초등 영어 기초를 확인하는 방법",
    ),
    "부발읍": (
        "예비 중학생을 위한",
        "예비 중학생의 초등 영어 기초를 점검하는 방법",
    ),
    "시지동": (
        "중학교 영어까지 준비하는 학습 설계",
        "초등 영어 기초를 중학교 영어로 연결하는 학습 설계",
    ),
    "상남동": (
        "중1 첫 시험까지 준비하는 영어 학습",
        "초6 영어에서 중1 첫 시험까지 준비하는 학습 순서",
    ),
    "중화산동": (
        "중등 영어까지 준비",
        "초등 영어 읽기·어휘를 중등 영어 준비로 연결하는 방법",
    ),
}

EXPECTED_BLANK_GRADE_LOCALITIES = {
    "운양동", "고잔동", "초지동", "옥정동", "옥정신도시", "교하", "석사동", "퇴계동",
}
...
AWKWARD_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("wrong_date_particle", re.compile(r"날짜(?:이|을|은|과)(?=\s|[가-힣])")),
    ("duplicated_error_recheck", re.compile(r"오답\s+원인·다시\s+확인(?:의\s+확인|\s+재확인)")),
    ("duplicated_school_school", re.compile(r"학교의\s+학교")),
    ("awkward_start_adaptation", re.compile(r"처음\s+시작\s+적응")),
    ("nested_current_material", re.compile(r"(?:최근\s+교재와\s+학습지|[가-힣]+\s+교재)에서\s+[‘\"][^’\"]*현재\s+교재(?:와\s+학습지)?에서")),
    ("nested_new_sentence", re.compile(r"새\s+문장에서\s+[‘\"][^’\"]*새\s+문장(?:에서도|으로)")),
    ("duplicated_new_activity_context", re.compile(r"같은\s+기능의\s+새\s+자료에서\s+[‘\"][^’\"]*같은\s+기능의\s+새\s+활동")),
    ("duplicated_order_action", re.compile(r"순서를\s+정하기[’\"]의\s+순서를")),
    ("duplicated_blocked_area", re.compile(r"먼저\s+막힌\s+영역에\s+[‘\"]가장\s+먼저\s+막힌")),
    ("wrong_rule_particle", re.compile(r"규칙\s+한\s+줄과\s+맞는")),
    ("wrong_single_check_phrase", re.compile(r"한\s+학습\s+확인")),
    ("ambiguous_missing_word_order", re.compile(r"빠진\s+낱말·순서")),
    ("generic_school_group_as_name", re.compile(r"기타사립초")),
    ("stiff_preserve_command", re.compile(r"보존(?:하세요|합니다)")),
    ("wrong_new_passage_school_unit", re.compile(r"새\s+지문의\s+학교\s+영어·단원평가")),
    ("wrong_universal_sentence_recheck", re.compile(r"[’\"]\s+뒤\s+(?:재확인할\s+)?새\s+문장에서")),
    ("wrong_sentence_recheck_label", re.compile(r"(?:복습·과제\s+루틴|학습\s+우선순위|질문·혼자\s+설명|듣기·말하기|소리·문자\s+연결|근거\s+독해|학교\s+영어·단원평가)(?:은|는)\s+새\s+문장의\s+재확인일")),
    ("mixed_intent_record", re.compile(r"(?:복습·과제\s+루틴|학습\s+우선순위|질문·혼자\s+설명|학교\s+영어·단원평가|근거\s+독해|문장\s+읽기)\s+기록에는[^.!?]{0,260}다시\s+확인한\s+날짜")),
    ("generic_explanation_recheck", re.compile(r"(?:재확인\s+날\s+혼자\s+설명한\s+범위|아이가\s+혼자\s+설명할\s+날짜|며칠\s+뒤\s+각\s+영역에서\s+아이가\s+혼자\s+설명한\s+범위)")),
    ("priority_as_sentence_attribute", re.compile(r"학교\s+교재와\s+새\s+글에서\s+학습\s+우선순위의\s+어려움이\s+드러난\s+문장")),
    ("school_intent_in_home_material", re.compile(r"학교\s+자료의[^.!?]{0,180}평소\s+자료의\s+학교\s+영어·단원평가\s+기록")),
    ("wrong_recheck_result_particle", re.compile(r"(?:낱말|문장|표현|지점|분량)(?:를|가|와)(?=\s|[가-힣])")),
    ...
    ("broken_phrase", re.compile(r"(?:수\s*있은|묻은\s*질문|찾은\s*자리|맞은\s*순서|함께\s*읽은\s*것입니다|낱말의\s*원인|문장의\s*원인|학습\s*루틴의\s*원인|발음\s*실수의\s*근거|평소에는\s+평소|준비\s+준비(?:까지|의)|점검할\s*학습\s+기록\s+점검|준비피드백|문장\s+고민|진단과\s+학교\s+영어\s+준비|집중\s+점검학습|과제점검학습|영어피드백|오후학습|학습연습|학원학습|학습반복|학습클리닉|학원프로그램|준비종합점검|학습취약점|학습자립도|내신보충학습|학습진단|학습목표|학습기록|학습이력|대면학습|학습자율성|학습플래너|학습실전|화상학습|학습성취도|내신학습|선택\s+전\s+학원\s+변경\s+전|초등\s+영어학원의\s+초등\s+영어|최소\s+과제를\s+현재\s+시간표와\s+대조|참고\s+학교와\s+학습\s+우선순위\s+개설\s+범위|학습\s+우선순위와[^.!?]{0,24}의\s+우선순위를|학습\s+우선순위에\s+설명이\s+필요한지|학교\s+영어와\s+학습\s+습관을\s+함께(?=\s*[,.:]|</|$)|다음\s+학년\s+준비\s+점검과\s+학습\s+점검을\s+함께|시험\s+준비\s+점검까지\s+살피는\s+학습\s+점검|학습\s+기록\s+점검으로\s+공부\s+흐름\s+점검하기|(?:봄방학\s+영어\s+계획|학기\s+초\s+공부\s+습관|겨울방학\s+기초)를?\s+(?:세우는|다지는)(?=\s*[,.:]|</|$)|답은\s+성적표가\s+아니라\s+최근\s+시험지|다음\s+점검에서도\s+같은\s+시험지를\s+사용하되|시험지의\s+표시와\s+재풀이\s+결과를\s+각각\s+이어|센터\s+정보와[^.!?]{0,50}와[^.!?]{0,30}질문)")),
    ...
    ("duplicate_recheck", re.compile(r"다시\s+확인\s+확인")),
    ("consultation_focus_redundancy", re.compile(r"상담\s+전\s+[‘\"]상담\s+전에\s+볼")),
    ("duplicated_recent_material", re.compile(r"최근\s+교재와\s+학습지\s+한\s+장과\s+현재\s+교재")),
    ("duplicated_progress_material", re.compile(r"학교\s+진도표와\s+최근\s+교재와\s+학습지")),
    ("duplicated_mark_action", re.compile(r"[‘\"][^’\"]{1,160}표시한\s+기록[’\"]을\s+표시(?:하세요|하고|한\s+뒤)")),
    ("duplicated_student_school", re.compile(r"학생\s+학교의\s+학교\s+학습\s+일정")),
    ("intent_presented_as_service", re.compile(r"(?:수업의\s+현재\s+개설\s+여부|활동의\s+반\s+구성|의\s+수업\s+시간과\s+실제\s+개설\s+반)")),
    ("awkward_grade_application_join", re.compile(r"어느\s+학년에\s+적용할지와\s+실제\s+시간표")),
    ("same_sheet_new_material_conflict", re.compile(r"다음\s+점검에서도\s+같은\s+학습지를\s+사용하되")),
    ("checkpoint_noun_compared_to_record", re.compile(r"뒤\s+새\s+문장의\s+[‘\"][^’\"]{1,180}[’\"]를\s+첫\s+기록과\s+비교")),
    ("materials_declared_sufficient", re.compile(r"교재와\s+주간\s+계획표면\s+충분합니다")),
    ("missing_together_particle", re.compile(r"결과를\s+날짜와\s+남기세요")),
    ("duplicated_school_possessive", re.compile(r"실제\s+재학\s+학교의\s+학교\s+(?:진도표|학습\s+일정)")),
    ("duplicated_evidence_location", re.compile(r"확인\s+자료:[^.!?]{1,100}에서\s+[가-힣A-Za-z0-9]+\s+교재에서")),
    ("nominalized_schedule_join", re.compile(r"(?:하기의\s+실행일|는지의\s+재확인일)")),
    ("new_checkpoint_same_material_conflict", re.compile(r"일주일\s+뒤\s+[‘\"][^’\"]{1,220}(?:새\s+문장|새\s+활동|처음\s+보는\s+글)[^’\"]*[’\"][을를]\s+같은\s+자료로\s+확인")),
    ("question_checkpoint_used_as_action", re.compile(r"[‘\"][^’\"]{1,220}하는지[’\"]\s+뒤\s+첫날과\s+달라진\s+점")),
    ("duplicated_week_recheck", re.compile(r"(?:일주일\s+뒤|일주일\s+후|7일\s+뒤에는|7일째에는|며칠\s+뒤|재확인\s+날|다음\s+점검에서|다시\s+볼\s+날짜에는|마지막\s+날에는|재확인\s+때)[^.!?]{0,100}[‘\"]일주일\s+뒤")),
    ("misleading_word_order_change", re.compile(r"낱말\s+순서를\s+바꾸어\s+이해한\s+위치")),
    ("ambiguous_same_sentence_order", re.compile(r"낱말이\s+일부\s+바뀐\s+새\s+문장도\s+같은\s+순서로")),
    ("malformed_school_material_join", re.compile(r"학교\s+교재의\s+현재\s+단원,\s*학습지와")),
    ("ambiguous_vocabulary_record_join", re.compile(r"단어\s+확인\s+기록과\s+같은\s+낱말")),
    ("sentence_picture_collocation", re.compile(r"순서와\s+그림이\s+달라진\s+새\s+문장")),
    ("vague_transition_language", re.compile(r"(?:다음\s+학년의\s+조금\s+긴\s+활동|조금\s+어려운\s+예시\s+활동|짧은\s+글과\s+문장에\s+한\s+단계씩\s+적용하기)")),
    ("same_material_recheck", re.compile(r"같은\s+자료로\s+다시\s+확인")),
    ("marked_record_as_page_location", re.compile(r"표시한\s+기록[’\"]이\s+남은\s+쪽수와\s+날짜")),
    ("malformed_changed_point_list", re.compile(r"자료\s+위치·날짜·처음\s+판단한\s+이유가\s+달라진\s+지점")),
    ("conditional_then_unconditional_task", re.compile(r"첫\s+기록과\s+달라진\s+근거가\s+있을\s+때만")),
    ("plan_equals_completed_range", re.compile(r"계획은\s+[‘\"][^’\"]{1,220}[’\"][을를]\s+혼자\s+끝낸\s+범위로\s+정하세요")),
    ("learning_direction_as_metric", re.compile(r"처음\s+세운\s+학습\s+방향의\s+변화\s+여부")),
    ("missing_consultation_recipient", re.compile(r"재학\s+학교·학습\s+확인\s+범위·현재\s+진도를\s+직접\s+알려\s+주세요")),
    ("duplicate_recheck_label", re.compile(r"오답\s+원인·다시\s+확인의\s+확인일")),
    ("focus_record_needs_same_focus_material", re.compile(r"[‘\"]아이의\s+([^’\"]+)\s+기록[’\"]에\s+필요한\s+\1\s+자료")),
    ("vague_help_changed_location", re.compile(r"도움\s+뒤\s+달라진\s+위치를\s+보존하세요")),
    ("abstract_focus_connected_sentence", re.compile(r"[‘\"][^’\"]{4,180}[’\"]과\s+연결된\s+문장\s+한\s+개")),
    ("undefined_four_records", re.compile(r"학습\s+확인\s+뒤\s+네\s+기록을\s+비교")),
    (
        "blank_grade_phantom_reference",
        re.compile(r"(?:학년\s+)?표기를\s+확인할\s+수\s+없[\s\S]{0,220}?(?:이|해당)\s+학년\s+표기만으로"),
    ),
    ("duplicate_learning_join", re.compile(r"학습\s*,\s*학습\s+우선순위")),
    ("duplicate_question_join", re.compile(r"질문\s*,\s*질문·혼자\s+설명")),
    ("duplicate_elementary_english_heading", re.compile(r"(?:초등\s+영어:\s+영어|초등:\s+(?:초등\s+)?영어)")),
    ("duplicate_recheck_object", re.compile(r"오답\s+원인·다시\s+확인을\s+(?:다시\s+)?확인")),
    ("duplicate_recheck_current_line", re.compile(r"오답\s+원인·다시\s+확인의\s+현재선\s+확인")),
    ("duplicate_question_label", re.compile(r"질문·혼자\s+설명\s+(?:재확인\s+질문|상담\s+질문|확인\s+질문|질문)")),
    ("duplicate_explanation_label", re.compile(r"질문·혼자\s+설명\s+설명")),
    ("duplicate_check_basis", re.compile(r"기준[’']?을\s+확인\s+기준으로")),
    ("question_clause_answered_twice", re.compile(r"(?:하는|했는|인지)지[’']에[^.!?]{0,40}답(?:하게|하는지)")),
    ("duplicate_completion_trace", re.compile(r"완료\s+흔적을\s+남기기[’']를[^.!?]{0,40}완료\s+흔적을\s+남기")),
    ("intent_pair_forced_as_explanation", re.compile(r"부모의\s+도움\s+뒤에도[^.!?]{1,140}(?:을|를)\s+스스로\s+설명할\s+수\s+있는지")),
    ...
    ("remaining_focus_phrase", re.compile(r"(?:암기보다\s+이해를\s+세우는|학습\s+원인부터\s+살피는|틀린\s+문법을\s+실력으로|2등급을\s+위한\s+시험\s+대비|학습\s+기록\s+점검으로\s+완성하는\s+실력|실력의\s+방향을\s+바르게)")),
    ("unsupported_delivery_focus", re.compile(r"(?:화상\s*학습|대면\s*학습|매주\s+단어\s+테스트|개별지도\s+학습\s+안내|맞춤\s*코칭|맞춤\s*수업|밀착\s*관리)")),
    ("unspaced_learning_compound", re.compile(r"(?:학교시험|시험복습|시험분석|시험오답|시험전략|구문독해|목표점검|일정점검|내신진도점검|내신상담|문제풀이|오답노트|자기주도|내신시험)")),
...
UNSUPPORTED_OPERATION_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ...
    (
        ...
        re.compile(
            r"(?:소수\s*정예|개별\s*지도|맞춤(?:형)?\s*(?:코칭|수업|관리|지도)|"
            r"학생별\s*맞춤|개인별\s*맞춤|화상\s*수업|녹화\s*수업|온라인\s*수업|"
            r"대면\s*수업|오전\s*수업|주말\s*수업|그룹\s*수업|집중\s*수업|"
            r"정규반|집중반|시험\s*대비반|내신\s*특강|방학\s*특강|특강\s*수업|"
            r"방학\s*집중반|밀착\s*관리|매일\s*관리)"
        ),
    ),
    ("parent_reporting", re.compile(r"학부모(?:와|에게).{0,24}(?:공유|보고|전달)")),
    ("transport_service", re.compile(r"(?:차량\s*운행|등하원\s*차량|통학\s*차량|셔틀|픽업\s*서비스)")),
)
UNSUPPORTED_OUTCOME_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("score_improvement", re.compile(r"(?:성적|내신|등급|실력)\s*(?:향상|상승)|성적을\s*(?:올리|바꾸)")),
...
ALWAYS_FORBIDDEN_CONTAMINATION_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("math_subject", re.compile(r"(?:수학|연산(?!동)|도형|방정식|함수|계산\s*문제)")),
    ("high_school_level", re.compile(r"(?:예비\s*고1|고등\s*(?:영어|학교|학생|과정|어휘)|(?<![0-9A-Za-z가-힣])고[1-3](?![0-9A-Za-z가-힣]))")),
    ("csat_or_mock_exam", re.compile(r"(?:수능|모의고사|전국연합|학력평가|(?<![가-힣])모고(?![가-힣]))")),
    ("adult_exam", re.compile(r"(?:토익|토플|아이엘츠|성인\s*영어)")),
)
ELEMENTARY_ENGLISH_SIGNAL_TERMS = (
    "파닉스", "알파벳", "소리", "발음", "어휘", "단어", "문장", "읽기",
    "독해", "문법", "듣기", "말하기", "쓰기", "영작", "받아쓰기", "철자",
)
READER_META_TERMS = (
    "검색 노출",
    "SEO",
...
def middle_dot_label_chains(value: str) -> list[str]:
    ...
    issues: set[str] = set()
    for left in ELEMENTARY_ENGLISH_LABELS:
        for right in ELEMENTARY_ENGLISH_LABELS:
            if left == right:
                continue
            fused = f"{left}·{right}"
            if fused in value:
                issues.add(fused)
    for old_chain in ("초6·중1 전환·상담 진단·우선순위", "초6·중1 전환 준비·학습 우선순위"):
        if old_chain in value:
            issues.add(old_chain)
    return sorted(issues)


QUOTED_PARTICLE_RE = re.compile(
    r"[‘“'\"](?P<phrase>[^’”'\"]*?[가-힣])[’”'\"]\s*"
    r"(?P<particle>는|은|를|을|가|이|와|과)"
)
QUOTED_PARTICLE_EXPECTATIONS = {
    "는": False,
    "은": True,
    "를": False,
    "을": True,
...
def hangul_has_final_consonant(character: str) -> bool:
    """Return whether a modern Hangul syllable ends with a batchim."""

    return "가" <= character <= "힣" and (ord(character) - ord("가")) % 28 != 0


def wrong_quoted_phrase_particles(value: str) -> list[dict[str, str]]:
    """Find subject/object/comitative particles that disagree with a quote's last syllable."""

    issues: list[dict[str, str]] = []
    for match in QUOTED_PARTICLE_RE.finditer(value):
        phrase = normalize(match.group("phrase"))
        if not phrase:
            continue
        final_character = phrase[-1]
        particle = match.group("particle")
        expected_batchim = QUOTED_PARTICLE_EXPECTATIONS[particle]
        if hangul_has_final_consonant(final_character) != expected_batchim:
            issues.append(
                {
                    "text": normalize(match.group(0))[-80:],
                    "final": final_character,
                    "particle": particle,
                }
            )
    return issues
...
def blank_grade_confirmation_sentences(value: str) -> list[str]:
    """Return clear sentences that defer both current grade availability and schedule."""

    results: list[str] = []
    for sentence in sentence_values(value):
        if "학년" not in sentence:
            continue
        if not re.search(r"(?:시간표|수업\s*시간)", sentence):
            continue
        if not re.search(r"(?:현재|실제|가능|운영|개설)", sentence):
            continue
        if not re.search(r"(?:확인|문의|대조|물어)", sentence):
            continue
        if not re.search(r"(?:센터|상담|별도|따로|함께|각각)", sentence):
            continue
...
def validate_scope(
    ...
    state.check(
        ...
        CATEGORY_SLUG,
        "all and only the 371 elementary-English detail pages must be changed",
        {
...
def validate_sitemap(
    ...
    article_date_modified_by_url: dict[str, str],
    state: AuditState,
) -> dict[str, object]:
    ...
    state.check("technical.sitemap_target_scope", expected_urls <= set(after_urls), path, "one or more elementary-English URLs are absent from sitemap", sorted(expected_urls - set(after_urls)))
    ...
    prior_subject_urls = {
        ...
        if (
            ...
            and parts[0] == "과목별학원"
            and parts[1] in SITEMAP_PRESERVED_SUBJECT_SLUGS
        )
    }
    ...
    state.check(
        ...
        path,
        "a sitemap lastmod outside elementary-English and the four preserved subject categories changed",
    ...
    state.check(
        ...
        path,
        "all 371 revised elementary-English URLs must receive a fresh lastmod",
        {
            "changed": len(changed_lastmod),
            "missing": sorted(expected_urls - changed_lastmod)[:50],
        },
    )
    date_modified_mismatches = [
        {
            "url": url,
            "article_dateModified": article_date_modified_by_url.get(url, ""),
            "sitemap_lastmod": after_map.get(url, ""),
        }
        for url in sorted(expected_urls)
        if article_date_modified_by_url.get(url, "") != after_map.get(url, "")
    ]
    state.check(
        "technical.article_date_modified_sitemap_parity",
        not date_modified_mismatches,
        path,
        "Article dateModified does not match the corresponding sitemap lastmod",
        date_modified_mismatches[:50],
    )
    return {
        "urls": len(after_urls),
        ...
        "article_date_modified_matches": len(expected_urls) - len(date_modified_mismatches),
        "article_date_modified_mismatches": len(date_modified_mismatches),
    }
...
def validate_baseline_contract(
    ...
    for field_name in before:
        ...
        if field_name == "local_info":
            # ``기타사립초`` is a source grouping label, not a school name.
            # Removing only that token is an approved factual correction; all
            # actual schools and the rest of the card remain immutable.
            ...
            for value in (before_value, after_value):
                ...
                for label, field_value in fields.items():
                    if "학교 참고" in label:
                        fields[label] = normalize(
                            str(field_value).replace("기타사립초", "")
                        )
                current["fields"] = fields
                normalized_values.append(current)
            before_value, after_value = normalized_values
        state.check(
...
def validate_meta_and_structure(
    ...
    locality = title.removesuffix(" 초등 영어학원")
    state.check("seo.description_identity", locality in description and "초등 영어학원" in description, path, "description lacks locality/category identity")
...
def validate_facts_and_schema(
    ...
    visible_grade_values = [value for value in re.split(r"[·,/\s]+", visible_grade_map.get("영어", "")) if re.fullmatch(r"초[1-6]", value)]
    ...
    if not expected_grades:
        ...
        confirmation_sentences = blank_grade_confirmation_sentences(authored_text)
        state.check(
            "facts.blank_grade_confirmation_sentence",
            bool(confirmation_sentences),
            path,
            "a blank-grade page must clearly say that current available grades and the actual schedule require separate confirmation",
        )
        phantom_grade_reference = bool(re.search(r"(?:이|해당)\s+학년\s+표기만으로", authored_text))
        state.check(
            "facts.blank_grade_no_phantom_reference",
            not phantom_grade_reference,
            path,
            "a blank-grade page refers to a grade notation that the supplied data does not contain",
        )
    ...
    if not expected_schools:
        ...
        state.check(
            "facts.blank_school_scenario_context",
            "참고 학교와 실제 재학 학교가 다른 경우" not in authored_text,
            path,
            "a blank-school page assumes a supplied reference school exists",
        )
        invented_school_names = sorted(
            {
                match.group(0)
                for match in re.finditer(
                    r"(?<![0-9A-Za-z가-힣])(?:[가-힣]{2,20}초등학교|[가-힣]{2,20}초)(?![0-9A-Za-z가-힣])",
                    authored_text,
                )
                if match.group(0) != "초등학교"
                and not match.group(0).endswith("기초")
                and match.group(0) not in all_schools
            }
        )
        state.check(
    ...
    grade_mentions = set(re.findall(r"(?<![0-9A-Za-z가-힣])초[1-6](?![0-9A-Za-z가-힣])", authored_text))
...
def validate_schema_content_contract(
    ...
    expected_breadcrumb = [
        ...
        (3, "초등 영어학원", absolute_route(CATEGORY_SLUG)),
        (4, title, canonical),
    ]
...
def validate_natural_reader_value(
    ...
    state.check("aeo.summary_identity", locality in summary_text and "초등 영어" in summary_text, path, "answer-first summary lacks locality/category identity")
    ...
    if focus:
        state.check(
            "aeo.summary_focus_workflow",
            focus in summary_text
            and all(
                marker in summary_text
                for marker in ("확인 자료:", "이번 주 행동:", "재확인 기준:")
            ),
            path,
            "the answer-first summary must connect the page focus to evidence, action and recheck",
            summary_text[:420],
        )
        ...
        state.check(
            ...
            len(linked_focus_sections) >= 1,
            path,
            "the page-specific focus must also appear in manuscript paragraph text and connect to evidence, action and checkpoint signals; the answer-first summary is checked separately",
            {
                "focus": focus,
        ...
        repeated_nonfocus_quotes = [
            {"text": text, "count": count}
            for text, count in Counter(
                match.group(1)
                for match in re.finditer(r"‘([^’]{15,})’", authored_text)
                if normalize(match.group(1)) != focus
            ).most_common()
            if count > 4
        ]
        state.check(
            "reader.nonfocus_quote_repetition",
            not repeated_nonfocus_quotes,
            path,
            "the same long evidence, action or checkpoint phrase is repeated more than four times within one page",
            repeated_nonfocus_quotes[:10],
        )
    ...
    wrong_quote_particles = wrong_quoted_phrase_particles(authored_text)
    if wrong_quote_particles:
        natural_issues.append({"kind": "wrong_quoted_phrase_particle", "values": wrong_quote_particles[:20]})
    ...
    if len(sections) >= 4:
        ...
        if "초등 영어 전환 질문" in facts_heading:
            natural_issues.append({
                "kind": "reversed_transition_heading",
                "heading": facts_heading,
            })
        if "초등학교 입학 준비 질문" in facts_heading:
            natural_issues.append({
                "kind": "diluted_entry_heading",
                "heading": facts_heading,
            })
    focus_compact = re.sub(r"\s+", "", focus)
    transition_context = "초6" in expected_grades and any(
        marker in focus_compact
        for marker in (
            "예비중", "중1", "중학교입학", "중학교첫", "중학교영어", "중학교준비",
            "중등영어", "중등전환", "초등중등전환", "첫시험",
        )
    )
    # Locality is source-backed entity data rather than editorial subject
    # language. 별내신도시 legitimately contains the substring "내신".
    authored_for_contamination = authored_text.replace(locality, "")
    contamination_hits: list[dict[str, object]] = []
    for label, pattern in ALWAYS_FORBIDDEN_CONTAMINATION_PATTERNS:
        hits = sorted(set(match.group(0) for match in pattern.finditer(authored_for_contamination)))
        if hits:
    ...
    if middle_hits and not transition_context:
        contamination_hits.append({"kind": "middle_school_context_outside_transition_intent", "values": middle_hits[:20]})
    unsupported_middle_grades = sorted(
        set(
            match.group(0)
            for match in re.finditer(r"(?<![0-9A-Za-z가-힣])중[2-3](?![0-9A-Za-z가-힣])", authored_text)
        )
    )
    if unsupported_middle_grades:
        contamination_hits.append({"kind": "unsupported_middle_grade", "values": unsupported_middle_grades})
    state.check(
        "natural.no_foreign_subject_level_assessment",
        not contamination_hits,
        path,
        "Math, high-school/adult-exam, CSAT/mock-exam or invalid middle-school transition contamination was found",
        contamination_hits,
    )
    subject_signals = sorted(term for term in ELEMENTARY_ENGLISH_SIGNAL_TERMS if term in authored_text)
    state.check(
        "reader.elementary_english_signal_density",
        len(subject_signals) >= 3,
        path,
        "fewer than three concrete elementary-English learning signals are present",
        subject_signals,
    )
    ...
    state.check(
        ...
        path,
        "unsupported class format, delivery method, custom program or parent-reporting operation was claimed",
...
def build_page_record(
    ...
    if locality in SOURCE_H1_CONTAMINATION_CONTRACT:
        ...
        state.check(
            ...
            path,
            "the audited elementary-English replacement focus for a contaminated source H1 is absent",
            safe_focus,
        )
    if locality in SOURCE_MIDDLE_TRANSITION_CONTRACT:
        _source_fragment, safe_transition_focus = SOURCE_MIDDLE_TRANSITION_CONTRACT[locality]
        state.check(
            "source.middle_transition_safe_focus",
            safe_transition_focus in authored_text,
            path,
            "the audited elementary-to-middle transition focus is absent",
            safe_transition_focus,
        )
    ...
    record = PageRecord(
        ...
        title=title,
        canonical=canonical,
        article_date_modified=normalize(
            str((node_for(nodes, "Article") or {}).get("dateModified", ""))
        ),
...
def validate_corpus(records: list[PageRecord], state: AuditState) -> dict[str, object]:
    ...
    state.check(
        "duplicates.masked_pair_count",
        len(pair_scores) == EXPECTED_PAIR_COUNT == 68_635,
        CATEGORY_SLUG,
        "exhaustive masked comparison did not cover all 68,635 page pairs",
        len(pair_scores),
    )
...
def recommended_samples(records: list[PageRecord], corpus: dict[str, object]) -> list[dict[str, object]]:
    ...
    for record in records:
        if record.blank_grades:
            reasons[record.relative].add("blank elementary-English grades")
...
def main() -> int:
    ...
    parser.add_argument("--baseline-ref", required=True, help="Git commit immediately before the elementary-English rewrite")
    ...
    blank_grade_localities = {
        ...
        if not relevant_grades(config, center, "영어")
    }
    ...
    state.check(
        ...
        "center CSV",
        "elementary-English blank-grade rows differ from the audited 8-row source contract",
        {"actual": sorted(blank_grade_localities), "expected": sorted(EXPECTED_BLANK_GRADE_LOCALITIES)},
    )
    state.check(
        "facts.blank_school_source_contract",
        blank_school_localities == EXPECTED_BLANK_SCHOOL_LOCALITIES,
        "center CSV",
        "elementary-English blank-school rows differ from the audited 74-row source contract",
        {"actual": sorted(blank_school_localities), "expected": sorted(EXPECTED_BLANK_SCHOOL_LOCALITIES)},
    )
    ...
    state.check("scope.filesystem", actual_paths == expected_paths, CATEGORY_SLUG, "elementary-English detail filesystem scope differs from the 371 CSV rows", {"actual": len(actual_paths), "expected": len(expected_paths), "missing": [display_path(path) for path in sorted(expected_paths - actual_paths)], "extra": [display_path(path) for path in sorted(actual_paths - expected_paths)]})
    ...
    detected_hard_contamination_rows: list[str] = []
    transition_rows: list[str] = []
    detected_middle_transition_rows: list[str] = []
    source_h1_values: list[str] = []
    source_h1_locality_mismatches: list[str] = []
    for center, value in zip(centers, source_rows):
        ...
        source_h1_values.append(source_h1)
        if locality not in source_h1:
            source_h1_locality_mismatches.append(locality)
        if any(pattern.search(source_h1) for _label, pattern in ALWAYS_FORBIDDEN_CONTAMINATION_PATTERNS):
            detected_hard_contamination_rows.append(locality)
        ...
        transition_contract = SOURCE_MIDDLE_TRANSITION_CONTRACT.get(locality)
        if transition_contract:
            transition_fragment, _safe_transition_focus = transition_contract
            has_transition = transition_fragment in source_h1
            state.check(
                "source.middle_transition_contract",
                has_transition and "초6" in relevant_grades(config, center, "영어"),
                config.source_path,
                f"the audited elementary-to-middle transition contract drifted for {locality}",
                {
                    "expected_fragment": transition_fragment,
                    "actual_h1": source_h1,
                    "grades": relevant_grades(config, center, "영어"),
                },
            )
            if has_transition:
                transition_rows.append(locality)
    state.check(
        "source.polluted_h1_count",
        set(contamination_rows) == set(SOURCE_H1_CONTAMINATION_CONTRACT),
        config.source_path,
        "the nine known wrong-subject, wrong-level or wrong-assessment source H1 rows were not identified exactly",
        {"actual": sorted(contamination_rows), "expected": sorted(SOURCE_H1_CONTAMINATION_CONTRACT)},
    )
    state.check("source.h1_count", len(source_h1_values) == EXPECTED_DETAILS and all(source_h1_values), config.source_path, "source workbook must provide one non-empty H1 for every row", len(source_h1_values))
    state.check("source.h1_unique", len(set(source_h1_values)) == EXPECTED_DETAILS, config.source_path, "source workbook H1 values are not all unique", len(set(source_h1_values)))
    state.check("source.h1_locality_identity", not source_h1_locality_mismatches, config.source_path, "one or more source H1 values omit their row locality", source_h1_locality_mismatches)
    state.check(
        "source.polluted_h1_detection_scope",
        set(detected_hard_contamination_rows) == set(SOURCE_H1_CONTAMINATION_CONTRACT),
        config.source_path,
        "automatic source H1 contamination detection differs from the nine-row contract",
        {
            "actual": sorted(detected_hard_contamination_rows),
            "expected": sorted(SOURCE_H1_CONTAMINATION_CONTRACT),
        },
    )
    state.check(
        "source.middle_transition_count",
        set(transition_rows) == set(SOURCE_MIDDLE_TRANSITION_CONTRACT),
        config.source_path,
        "the seven audited elementary-to-middle transition H1 rows were not identified exactly",
        {"actual": sorted(transition_rows), "expected": sorted(SOURCE_MIDDLE_TRANSITION_CONTRACT)},
    )
    state.check(
        "source.middle_transition_detection_scope",
        set(detected_middle_transition_rows) == set(SOURCE_MIDDLE_TRANSITION_CONTRACT),
        config.source_path,
        "automatic source H1 transition detection differs from the seven-row contract",
        {
            "actual": sorted(detected_middle_transition_rows),
            "expected": sorted(SOURCE_MIDDLE_TRANSITION_CONTRACT),
        },
    )
    ...
    state.check("facts.blank_grade_rendered_count", sum(record.blank_grades for record in records) == 8, CATEGORY_SLUG, "rendered blank-grade page count is not 8", sum(record.blank_grades for record in records))
    state.check("facts.blank_school_rendered_count", sum(record.blank_schools for record in records) == 74, CATEGORY_SLUG, "rendered blank-school page count is not 74", sum(record.blank_schools for record in records))
    state.check("technical.representative_unique", len(representative_digests) == EXPECTED_DETAILS and len(set(representative_digests)) == EXPECTED_DETAILS, CATEGORY_SLUG, "representative image contents are not unique within elementary English", {"files": len(representative_digests), "unique": len(set(representative_digests))})
    ...
    sitemap_metrics = validate_sitemap(
        baseline,
        expected_urls,
        {record.canonical: record.article_date_modified for record in records},
        state,
    )
    ...
    report: dict[str, object] = {
        ...
        "contract": {
            ...
            "exhaustive_pair_count": EXPECTED_PAIR_COUNT,
            "blank_grade_rows": len(EXPECTED_BLANK_GRADE_LOCALITIES),
            "blank_school_rows": len(EXPECTED_BLANK_SCHOOL_LOCALITIES),
            "hard_source_contamination_rows": len(SOURCE_H1_CONTAMINATION_CONTRACT),
            "elementary_to_middle_transition_rows": len(SOURCE_MIDDLE_TRANSITION_CONTRACT),
