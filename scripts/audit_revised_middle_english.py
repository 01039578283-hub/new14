#!/usr/bin/env python3
"""Strict release audit for the 371 revised middle-English detail pages.
...
# These workbook rows have a clearly wrong subject or grade in the source H1.
# Keep the contract independent from the generator so source drift cannot
# silently restore those fragments to middle-English pages.
SOURCE_H1_CONTAMINATION_CONTRACT: dict[str, tuple[str, str]] = {
    "후곡마을": (
        "고3 영어",
        "기초의 빈틈과 오답 흐름을 점검하는 방법",
    ),
    "동춘동": (
        "봄방학 수학 공부법",
        "봄방학 동안 중등 영어 기초와 학습 목표를 정리하는 방법",
    ),
    "연수동": (
        "고2 수능 대비",
        "독해를 근거 중심으로 점검하는 방법",
...
AWKWARD_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ...
    ("broken_phrase", re.compile(r"(?:수\s*있은|묻은\s*질문|찾은\s*자리|맞은\s*순서|함께\s*읽은\s*것입니다|개념의\s*원인|공식의\s*원인|학습\s*루틴의\s*원인|계산\s*실수의\s*근거|평소에는\s+평소|준비\s+준비(?:까지|의)|점검할\s*학습\s+기록\s+점검|준비피드백|문장제\s+고민|진단과\s+학교\s+시험\s+준비|집중\s+점검학습|과제점검학습|영어피드백|오후학습|학습연습|학원학습|학습반복|학습클리닉|학원프로그램|준비종합점검|학습취약점|학습자립도|내신보충학습|학습진단|학습목표|학습기록|학습이력|대면학습|학습자율성|학습플래너|학습실전|화상학습|학습성취도|내신학습|선택\s+전\s+학원\s+변경\s+전|중등\s+영어학원의\s+중등\s+영어|최소\s+과제를\s+현재\s+시간표와\s+대조|참고\s+학교와\s+학습\s+우선순위\s+개설\s+범위|학습\s+우선순위와[^.!?]{0,24}의\s+우선순위를|학습\s+우선순위에\s+설명이\s+필요한지|시험\s+대비와\s+학습\s+습관을\s+함께(?=\s*[,.:]|</|$)|다음\s+학년\s+준비\s+점검과\s+학습\s+점검을\s+함께|시험\s+준비\s+점검까지\s+살피는\s+학습\s+점검|학습\s+기록\s+점검으로\s+공부\s+흐름\s+점검하기|(?:봄방학\s+영어\s+계획|학기\s+초\s+공부\s+습관|겨울방학\s+기초)를?\s+(?:세우는|다지는)(?=\s*[,.:]|</|$)|답은\s+성적표가\s+아니라\s+최근\s+시험지|다음\s+점검에서도\s+같은\s+시험지를\s+사용하되|시험지의\s+표시와\s+재풀이\s+결과를\s+각각\s+이어|센터\s+정보와[^.!?]{0,50}와[^.!?]{0,30}질문)")),
    ...
    ("unsupported_delivery_focus", re.compile(r"(?:화상\s*학습|대면\s*학습|매주\s+단어\s+테스트|개별지도\s+학습\s+안내)")),
...
def middle_dot_label_chains(value: str) -> list[str]:
    ...
    issues: set[str] = set()
    for left in MIDDLE_ENGLISH_LABELS:
        for right in MIDDLE_ENGLISH_LABELS:
            if left == right:
                continue
            fused = f"{left}·{right}"
            if fused in value:
                issues.add(fused)
    for old_chain in ("중3·고등 전환·상담 진단·우선순위", "고등 영어 전환 준비·학습 우선순위"):
...
def validate_scope(
    ...
    state.check(
        ...
        CATEGORY_SLUG,
        "all and only the 371 middle-English detail pages must be changed",
        {
    ...
    return {
        ...
        "preserved_prior_high_subject_details": len(prior_detail_changes),
...
def validate_sitemap(
    ...
    state.check("technical.sitemap_target_scope", expected_urls <= set(after_urls), path, "one or more middle-English URLs are absent from sitemap", sorted(expected_urls - set(after_urls)))
    ...
    prior_high_subject_urls = {
        url
        for url in set(after_map)
        if (
            len(parts := unquote(urlsplit(html.unescape(url)).path).strip("/").split("/")) == 3
            and parts[0] == "과목별학원"
            and parts[1] in {"고등영어학원", "고등수학학원"}
        )
    }
    allowed_changed_lastmod = expected_urls | prior_high_subject_urls
    state.check(
        "scope.sitemap_only_target_lastmod",
        changed_lastmod <= allowed_changed_lastmod,
        path,
        "a sitemap lastmod outside middle-English and preserved high-subject details changed",
    ...
    state.check(
        ...
        path,
        "all 371 revised middle-English URLs must receive a fresh lastmod",
        {
    ...
    return {
        ...
        "preserved_prior_high_subject_lastmods": len(changed_lastmod & prior_high_subject_urls),
...
def validate_meta_and_structure(
    ...
    locality = title.removesuffix(" 중등 영어학원")
    state.check("seo.description_identity", locality in description and "중등 영어학원" in description, path, "description lacks locality/category identity")
...
def validate_facts_and_schema(
    ...
    visible_grade_values = [value for value in re.split(r"[·,/\s]+", visible_grade_map.get("영어", "")) if re.fullmatch(r"중[1-3]", value)]
...
def validate_schema_content_contract(
    ...
    expected_breadcrumb = [
        ...
        (3, "중등 영어학원", absolute_route(CATEGORY_SLUG)),
        (4, title, canonical),
    ]
...
def validate_natural_reader_value(
    ...
    state.check("aeo.summary_identity", locality in summary_text and "중등 영어" in summary_text, path, "answer-first summary lacks locality/category identity")
    ...
    if len(sections) >= 4:
        ...
        if "중등 영어 전환 질문" in facts_heading:
            natural_issues.append({
    ...
    if any(marker in focus_compact for marker in ("예비중", "중1", "중학교첫", "첫시험")):
        if "고등 영어 전환 준비" in authored_text:
            natural_issues.append({
                "kind": "school_transition_intent_conflict",
                "focus": focus,
                "conflicting_label": "고등 영어 전환 준비",
            })
...
def build_page_record(
    ...
    if locality in SOURCE_H1_CONTAMINATION_CONTRACT:
        ...
        state.check(
            ...
            path,
            "the audited middle-English replacement focus for a contaminated source H1 is absent",
            safe_focus,
        )
...
def recommended_samples(records: list[PageRecord], corpus: dict[str, object]) -> list[dict[str, object]]:
    ...
    for record in records:
        if record.blank_grades:
            reasons[record.relative].add("blank middle-English grades")
...
def main() -> int:
    ...
    parser.add_argument("--baseline-ref", required=True, help="Git commit immediately before the middle-English rewrite")
    ...
    state.check("scope.filesystem", actual_paths == expected_paths, CATEGORY_SLUG, "middle-English detail filesystem scope differs from the 371 CSV rows", {"actual": len(actual_paths), "expected": len(expected_paths), "missing": [display_path(path) for path in sorted(expected_paths - actual_paths)], "extra": [display_path(path) for path in sorted(actual_paths - expected_paths)]})
    ...
    state.check(
        ...
        config.source_path,
        "the three known wrong-subject or wrong-grade source H1 rows were not identified exactly",
    ...
    state.check("technical.representative_unique", len(representative_digests) == EXPECTED_DETAILS and len(set(representative_digests)) == EXPECTED_DETAILS, CATEGORY_SLUG, "representative image contents are not unique within middle English", {"files": len(representative_digests), "unique": len(set(representative_digests))})
