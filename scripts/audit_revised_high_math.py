#!/usr/bin/env python3
"""Strict release audit for the 371 revised high-Math detail pages.
...
PRIOR_DETAIL_PREFIX = "과목별학원/고등영어학원/"

# Five workbook rows carried an English/middle-school phrase in the original
# H1 even though the destination category is high-school mathematics.  Keep
# this contract independent from the generator so a future source-normalizing
# change cannot silently reintroduce the contaminated wording.
SOURCE_H1_CONTAMINATION_CONTRACT: dict[str, tuple[str, str]] = {
    "방화동": (
        "중1 첫 시험 준비를 위한 학습관리",
        "고등 수학 첫 내신을 준비하는 기본기 점검",
    ),
    "신중동": (
        "장문 독해와 성적 설계까지",
        "단원별 취약점과 풀이 과정을 연결하는 방법",
    ),
    "성복동": (
        "중등 내신부터 고등 학습 흐름 세우기",
        "중등 선수 개념을 고등 수학의 풀이 과정으로 연결하는 방법",
    ),
    "야당동": (
        "구문독해와 학습습관까지 함께 관리",
        "문제 이해와 수학 학습 습관을 함께 점검하는 방법",
    ),
    "자은동": (
        "중1 첫 시험을 준비하는 학습 설계",
        "고등 수학 시험 범위와 오답 원인을 함께 점검하는 방법",
    ),
}
...
AWKWARD_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ...
    ("broken_phrase", re.compile(r"(?:수\s*있은|묻은\s*질문|찾은\s*자리|맞은\s*순서|함께\s*읽은\s*것입니다|개념의\s*원인|공식의\s*원인|학습\s*루틴의\s*원인|계산\s*실수의\s*근거)")),
)
READER_META_TERMS = (
    "검색 노출",
    "SEO",
...
def validate_scope(
    ...
    prior_detail_changes = {
        ...
        if relative.startswith(PRIOR_DETAIL_PREFIX)
    ...
    state.check(
        ...
        CATEGORY_SLUG,
        "all and only the 371 high-Math detail pages must be changed",
        {
    ...
    return {
        ...
        "preserved_prior_high_english_details": len(prior_detail_changes),
...
def validate_sitemap(
    ...
    state.check("technical.sitemap_target_scope", expected_urls <= set(after_urls), path, "one or more high-Math URLs are absent from sitemap", sorted(expected_urls - set(after_urls)))
    ...
    allowed_changed_lastmod = expected_urls | prior_high_english_urls
    state.check(
        "scope.sitemap_only_target_lastmod",
        changed_lastmod <= allowed_changed_lastmod,
        path,
        "a sitemap lastmod outside high-Math and preserved high-English details changed",
    ...
    state.check(
        ...
        path,
        "all 371 revised high-Math URLs must receive a fresh lastmod",
        {
    ...
    return {
        ...
        "preserved_prior_high_english_lastmods": len(changed_lastmod & prior_high_english_urls),
...
def validate_meta_and_structure(
    ...
    locality = title.removesuffix(" 고등 수학학원")
    state.check("seo.description_identity", locality in description and "고등 수학학원" in description, path, "description lacks locality/category identity")
...
def validate_facts_and_schema(
    ...
    visible_grade_values = [value for value in re.split(r"[·,/\s]+", visible_grade_map.get("수학", "")) if re.fullmatch(r"고[1-3]", value)]
...
def validate_schema_content_contract(
    ...
    expected_breadcrumb = [
        ...
        (3, "고등 수학학원", absolute_route(CATEGORY_SLUG)),
        (4, title, canonical),
    ]
...
def validate_natural_reader_value(
    ...
    state.check("aeo.summary_identity", locality in summary_text and "고등 수학" in summary_text, path, "answer-first summary lacks locality/category identity")
...
def build_page_record(
    ...
    if locality in SOURCE_H1_CONTAMINATION_CONTRACT:
        ...
        state.check(
            ...
            path,
            "the audited high-Math replacement focus for a contaminated source H1 is absent",
            safe_focus,
        )
    ...
    record = PageRecord(
        ...
        blank_grades=not bool(relevant_grades(config, center, "수학")),
...
def recommended_samples(records: list[PageRecord], corpus: dict[str, object]) -> list[dict[str, object]]:
    ...
    for record in records:
        if record.blank_grades:
            reasons[record.relative].add("blank high-Math grades")
...
def main() -> int:
    ...
    parser.add_argument("--baseline-ref", required=True, help="Git commit immediately before the high-Math rewrite")
    ...
    state.check("scope.filesystem", actual_paths == expected_paths, CATEGORY_SLUG, "high-Math detail filesystem scope differs from the 371 CSV rows", {"actual": len(actual_paths), "expected": len(expected_paths), "missing": [display_path(path) for path in sorted(expected_paths - actual_paths)], "extra": [display_path(path) for path in sorted(actual_paths - expected_paths)]})
    ...
    state.check(
        ...
        config.source_path,
        "the five known wrong-subject source H1 rows were not identified exactly",
    ...
    state.check("technical.representative_unique", len(representative_digests) == EXPECTED_DETAILS and len(set(representative_digests)) == EXPECTED_DETAILS, CATEGORY_SLUG, "representative image contents are not unique within high Math", {"files": len(representative_digests), "unique": len(set(representative_digests))})
