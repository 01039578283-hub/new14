"""Non-mutating local/public release checks for site14 title changes."""
...
DEPLOYMENT_EXCLUSIONS = ["tools/", ".vercel", ".git", ".env*", "scripts/__pycache__/"]
...
def verify_source(row,source,public=False,rarity=None):
    errors=[]
    ...
    if row["kind"]!="category":
        ...
        suffix,evidence,mode=select_detail(dict(row,rel=row["path"],blocks=blocks),rarity)
        if (suffix,evidence,mode)!=(row["suffix"],row["evidence"],row["selectionMode"]):
            errors.append("learning suffix and evidence cannot be reproduced from the body")
...
def fetch(url):
    request=Request(url,headers={"User-Agent":"Site14-Title-Release-Check/1.0"})
...
def check_baseline(report):
    ...
    with GitBaseline() as baseline:
        ...
        protected=report["protectedHtml"]+["sitemap.xml","robots.txt","vercel.json","assets/site14.css","assets/site14.js","assets/national-anchor-toc.css"]
    ...
    for item in ["assets/generated/*.png","scripts/_*.json","scripts/_*.png","_direct-file-mobile.png"]+DEPLOYMENT_EXCLUSIONS:
...
def main():
    ...
    def check(row):
        try:
            ...
            return dict(path=row["path"],url=row["url"],title=title_of(source),httpStatus=200 if args.public else None,
                        errors=verify_source(row,source,args.public,report["selectionRarity"].get(row["group"])))
    ...
    presentation=[]
    if args.public:
        for path in ("index.html","assets/site14.css","assets/site14.js","과목별학원/index.html","robots.txt"):
            route="/" if path=="index.html" else "/과목별학원/" if path=="과목별학원/index.html" else "/"+path
            try:
    ...
    output=dict(mode="public" if args.public else "local",checked=len(results),failures=len(failures),
                globalErrors=global_errors,discovery=counts,presentation=presentation,tocPagesChecked=sum(r["kind"]!="category" for r in rows),checkedAt=datetime.now(timezone.utc).isoformat(),results=results)
